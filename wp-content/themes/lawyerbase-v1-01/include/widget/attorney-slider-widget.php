<?php
/**
 * Plugin Name: Goodlayers Attorney Slider
 * Plugin URI: http://goodlayers.com/
 * Description: A widget that show attorney as slider( Specified by category ).
 * Version: 1.0
 * Author: Goodlayers
...
add_action( 'widgets_init', 'gdlr_attorney_slider_widget' );
if( !function_exists('gdlr_attorney_slider_widget') ){
	function gdlr_attorney_slider_widget() {
		register_widget( 'Goodlayers_Attorney_Slider' );
	}
}

if( !class_exists('Goodlayers_Attorney_Slider') ){
	class Goodlayers_Attorney_Slider extends WP_Widget{

		// Initialize the widget
		function __construct() {
			parent::WP_Widget(
				'gdlr-attorney-slider-widget', 
				__('Goodlayers Attorney Slider Widget','gdlr_translate'), 
				array('description' => __('A widget that show lastest attorney in slider format', 'gdlr_translate')));  
		}
		...
		function widget( $args, $instance ) {
			...
			$query_args = array('post_type' => 'attorney', 'suppress_filters' => false);
			$query_args['posts_per_page'] = $num_fetch;
			$query_args['orderby'] = 'post_date';
			$query_args['order'] = 'desc';
			$query_args['paged'] = 1;
			$query_args['attorney_category'] = $category;
			$query_args['ignore_sticky_posts'] = 1;
			$query_args['post__not_in'] = array(get_the_ID());
			$query = new WP_Query( $query_args );
			
			if($query->have_posts()){
				echo '<div class="gdlr-widget-title-nav gdlr-nav-container">';
				echo '<i class="icon-angle-left gdlr-flex-prev"></i>';
				echo '<i class="icon-angle-right gdlr-flex-next"></i>';
				echo '<div class="clear"></div>';
				echo '</div>';
				
				echo '<div class="gdlr-attorney-slider-widget">';
				echo '<div class="flexslider" data-nav-container="gdlr-attorney-slider-widget" >';
				echo '<ul class="slides" >';
				while($query->have_posts()){ $query->the_post();
					$image_id = get_post_thumbnail_id();
					$attorney_option = json_decode(gdlr_decode_preventslashes(get_post_meta(get_the_ID(), 'post-option', true)), true);
					
					if( !empty($image_id) ){
						echo '<li>';
						echo gdlr_get_image($image_id, $thumbnail_size);
						echo '<div class="gdlr-attorney-slider-content">';
						echo '<div class="gdlr-attorney-content-title gdlr-sidebar-title-color">';
						echo '<a href="' . get_permalink() . '" >' . get_the_title() . '</a>';
						echo '</div>'; 
						echo '<div class="gdlr-attorney-content-position gdlr-sidebar-info-color">' . $attorney_option['position'] . '</div>'; 
						echo '</div>'; // gdlr-attorney-slider-content
						echo '</li>';
					}
				}
				echo '</ul>';
		...
		function form( $instance ) {
			...
			<p>
				...
				<?php 	
				$category_list = gdlr_get_term_list('attorney_category'); 
